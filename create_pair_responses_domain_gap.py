...
parser.add_argument("--fs_type", type=str, default="in")
parser.add_argument("--fs_num", type=int, default=6)
...
if args.useGT:
    ...
    prompt = ""
    if args.useFS:
        for tqa_data in fs_demon[args.fs_type][:args.fs_num]:
...
elif "truthful" in args.data_name:
    for question, mc1_targets in zip(train_dataset["question"][:num_query], train_dataset["mc1_targets"][:num_query]):
...
json_file = os.path.join(save_dir, f"{args.save_prefix}_num_{str(num_query)}_useGT_{str(args.useGT)}_useFS_{str(args.useFS)}_fs_type_{args.fs_type}_fs_num_{str(args.fs_num)}.json")
